...
public class MetricalLpcfgGenerator {
	...
	public void parseSong(JointModel jm, TimeTracker tt) {
		...
		int measureOffset = beats.get(0).getMeasure();
		int numMeasures = beats.get(beats.size() - 1).getMeasure() - measureOffset + 1;
		...
		for (int i = 0; i < beats.size(); i += this.notes32PerMeasure - beats.get(i).getBeat()) {
			Beat beat = beats.get(i);
			updateTimeSignature(tt.getNodeAtTime(beat.getTime()).getTimeSignature());
			
			int measure = beat.getMeasure() - measureOffset;
		...
		for (Voice voice : jm.getVoiceHypotheses().get(0).getVoices()) {
			...
			for (int i = 0; i < numMeasures; i++) {
				if (quantums[i] != null) {
					MetricalLpcfgTree tree = MetricalLpcfgTreeFactory.makeTree(quantums[i], beatsPerMeasure[i], subBeatsPerBeat[i]);
					if (!hasBegun) {
						hasBegun = true;
	...
	private void addNote(MidiNote note, List<Beat> beats, MetricalLpcfgQuantum[][] quantums,
		...
		// Add onset
		int measure = beat.getMeasure() - measureOffset;
		addQuantum(MetricalLpcfgQuantum.ONSET, quantums, measure, beat.getBeat(), notes32PerMeasure[measure]);
		
		// Add ties
		while (!beat.equals(offsetBeat)) {
			addQuantum(MetricalLpcfgQuantum.TIE, quantums, measure, beat.getBeat(), notes32PerMeasure[measure]);
